...
export default function Root() {
  ...
  const camerasList =
    topCameras?.length > 0 ? (
      topCameras.map((camera) => (
        <Box component="p" key={camera.id}>
          <Link to={`/cameras/${camera.id}`}>
            {camera.producer.name} {camera.model} ({camera.serialNumber})
          </Link>
        </Box>
      ))
    ) : (
  ...
  const lensesList =
    topLenses !== null && topLenses.length > 0 ? (
      topLenses.map((lens) => (
        <Box component="p" key={lens.id}>
          <Link to={`/lenses/${lens.id}`}>
            {lens.producer.name} {lens.model} ({lens.serialNumber})
          </Link>
        </Box>
      ))
    ) : (
  ...
  return (
    <Box component="div">
      <Grid container direction="column" spacing={1}>
              ...
              <Grid container spacing={1}>
                <Grid item md={6}>
                  <Card variant="outlined">
                    ...
                    <CardContent sx={{ m: 0, p: 1 }}>
                      <BasicLoader loading={topCameras === null}>{camerasList}</BasicLoader>
                    </CardContent>
                    <CardActions>
                ...
                <Grid item md={6}>
                  <Card variant="outlined">
                    ...
                    <CardContent sx={{ m: 0, p: 1 }}>
                      <BasicLoader loading={topLenses === null}>{lensesList}</BasicLoader>
                    </CardContent>
                    <CardActions>
        ...
        <Grid item>
          <Card variant="outlined">
            <CardContent>This is next card</CardContent>
          </Card>
        </Grid>
      </Grid>
    </Box>
